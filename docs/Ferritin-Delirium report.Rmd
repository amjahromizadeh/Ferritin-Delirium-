...
<div class="figcap">
  <b>Figure MR1. Causal estimates across MR estimators.</b>
...
cat(dl_btn("Download: Supp Table S1 (panel summary)", paste0("data/", S1_FN)))
cat(" ")
cat(dl_btn("Download: Supp Table S2 (BF+HEIDI probe-level hits)", paste0("data/", S2_FN)))
cat('</div>')
```
...
<div class="figcap"><b>SMR Figure 1.</b> Tissue × omics yield heatmap (sum of BF+HEIDI passing probes across panels), contrasting Ferritin’s broad cis-mediated signal landscape with Delirium’s sparse retained signals.</div> 
...
<div class="figcap"><b>SMR Figure 2.</b> Highest-yield Ferritin panels by number of BF+HEIDI passing probes, highlighting where cis-mediated associations are most concentrated.</div>
...
<div class="figcap"><b>SMR Figure 3.</b> Top Ferritin genes ranked by the most significant SMR association per gene (minimum p<sub>SMR</sub> across BF+HEIDI passing records).</div> 
...
<div class="figcap"><b>SMR Figure 4.</b> Delirium retained genes (best p<sub>SMR</sub> per gene) show a sparse profile dominated by the 19q13 locus signals.</div>
...
cat(kbl_nice(t1, caption = "Table 2-1. Delirium BF+HEIDI-retained SMR hits (all passing records)."))
cat('</div>')
# ---------
# Table 2-2: Strict cross-trait overlap (shared genes; APOE only here)
# ---------
...
cat('<div class="scroll-x">')
cat(kbl_nice(t2, caption = "Table 2-2. Strict cross-trait overlap under BF+HEIDI (shared genes)."))
cat('</div>')
# ---------
# Table 2-3: Tissue × omics yields (aggregated)
# ---------
t3 <- data.table::copy(tbl_yield_matrix)
cat(kbl_nice(t3, caption = "Table 2-3. Sum of BF+HEIDI passing probes by tissue × omics (aggregated across panels)."))
```

<details>
<summary>Table 2-4 (Supp): Supp Table S1 — panel-level summary (all 86 panels)</summary>
...
cat(kbl_nice(tS1, caption = "Table 2-4 (Supp). Supp Table S1 — SMR/HEIDI panel summary."))
cat('</div>')
```
...
<details> 
<summary>Table 2-5 (Supp): Supp Table S2 — BF+HEIDI passing hits (probe level)</summary>
...
cat('<div class="scroll-xy">')
cat(kbl_nice(tS2, caption = "Table 2-5 (Supp). Supp Table S2 — Bonferroni+HEIDI retained SMR hits."))
cat('</div>')
```
...
<details> 
<summary>Table 2-6 (preview): Ferritin top BF+HEIDI passing records (smallest p<sub>SMR</sub>)</summary>
...
cat('<div class="scroll-xy">')
cat(kbl_nice(t6, caption = "Table 2-6 (preview). Top Ferritin BF+HEIDI passing records ranked by p_SMR."))
cat('</div>')
...
# ---------------------------
# 2) SuSiE results (from susie md) — parse tables where available
# ---------------------------
...
cat(sprintf(
'<div class="card"><div class="kpi-label">High PP.H4 (≥0.80)</div><div class="kpi">%d</div><div class="kpi-sub">Evidence for shared causal variant</div></div>',
n_shared
))
cat(sprintf(
'<div class="card"><div class="kpi-label">High PP.H3 (≥0.80)</div><div class="kpi">%d</div><div class="kpi-sub">Evidence for distinct causal variants</div></div>',
n_distinct
))
cat(sprintf(
...
print(kbl_nice(
df_show,
caption = "Table C-1. Coloc.abf posterior summaries (PP.H3 vs PP.H4) across key loci and proxy panels."
))
...
print(p_pph4)
cat('<div class="figcap"><b>Figure C-1 (interpretation).</b> PP.H4 highlights strong locus-specific sharing for several Ferritin–QTL pairs (e.g., APOE pQTL, SLC11A2 eQTLGen, TF sQTL), while Delirium–APOE pQTL shows PP.H3≈1 with PP.H4≈0, supporting distinct causal variants in the same region.</div>')
...
print(kbl_nice(
sp,
caption = "Table C-2. SuSiE–coloc (Ferritin GWAS × APOE pQTL): top component pairs ranked by PP.H4.abf."
))
} else {
cat('<p><b>Missing table.</b> Could not find the “coloc.susie summary” pipe-table inside <code>Susie; process and results.md</code>.</p>')
}
...
cat('<div class="spacer"></div>')
print(p_susie_heat)
cat('<div class="figcap"><b>Figure C-2 (interpretation).</b> Component-pair heatmap: bright tiles mark pairs consistent with shared variant(s); off-diagonal high values would suggest cross-component sharing.</div>')
}
...
print(kbl_nice(
df_del_show,
caption = "Table C-3. SuSiE summary for Delirium GWAS × APOE pQTL: credible sets, lead SNPs, and the best observed component overlap (PP_shared)."
))
}
...
print(kbl_nice(
ce_sum,
caption = "Table C-4. Delirium × CEACAM19 (Cortex v8) coloc.abf summary table (as recorded in runlist)."
))
}
...
print(kbl_nice(
ce_top,
caption = "Table C-5. Top SNPs by PP.H4 for Delirium × CEACAM19 (Cortex v8)."
))
}
...
cat('<div class="spacer"></div>')
print(p_ce_pph4)
cat('<div class="figcap"><b>Figure C-3 (interpretation).</b> Two SNPs carry essentially all posterior mass under H4, implying a very small credible set in this window.</div>')
}
...
print(kbl_nice(
ce_lead,
caption = "Table C-6. Lead SNP details (aligned effects) for Delirium × CEACAM19 (Cortex v8)."
))
}
