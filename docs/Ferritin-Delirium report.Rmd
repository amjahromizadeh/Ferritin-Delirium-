...
## Integrated results across MR, SMR/HEIDI, coloc and SuSiE {.tabset .tabset-fade}
...
cat(dl_btn("Download: MR main table", "data/Table S1 MR Main.tsv"))
cat(kbl_nice(mr_tbl, caption = "Table S1. Two-sample MR estimates for ferritin → delirium across estimators."))

cat(sprintf('
<div class="figcap"><b>Table S1.</b> Two-sample MR causal estimates for ferritin → delirium across IVW, MR-Egger, and weighted median (OR per SD higher ferritin).</div>'))

# Table S2: diagnostics “scoreboard”
...
cat(sprintf('
<div class="figcap"><b>Table S2.</b> Instrument diagnostics for ferritin → delirium, including strength (F), heterogeneity (Cochran’s Q), directional pleiotropy (Egger intercept), directionality (Steiger), and MR-PRESSO outlier assessment.</div>'))


cat(dl_btn("Download: Table S3. MR heterogeneity", "data/Table S3 MR Heterogeneity.tsv"))
cat(sprintf('
<div class="figcap"><b>Table S3.</b> Cochran’s Q heterogeneity statistics for IVW and MR-Egger models (Q, df, p).</div>'))

cat(dl_btn("Download: Table S4. MR pleiotropy", "data/Table S4 MR Pleiotropy.tsv"))
cat(sprintf('
<div class="figcap"><b>Table S4.</b> MR-Egger intercept test for directional pleiotropy (intercept, SE, p).</div>'))

cat(dl_btn("Download: Table S5. MR Steiger", "data/Table S5 MR Steiger.tsv"))
cat(sprintf('
<div class="figcap"><b>Table S5.</b> Steiger filtering results comparing variance explained in exposure vs outcome (R²x, R²y) and inferred causal direction.</div>'))

cat(dl_btn("Download: Table S6. MR-PRESSO", "data/Table S6 MR PRESSO.tsv"))
cat(sprintf('
<div class="figcap"><b>Table S6.</b> MR-PRESSO raw and outlier-corrected causal estimates for ferritin → delirium, including OR.</div>'))
...
<div class="figcap">
  ...
  OR= Odds Ratio.Estimates are directionally consistent; primary IVW (randomeffects) interval includes the null.
</div>
...
render_fig <- function(title, filename, caption){
  ...
  # Use repo-relative path for HTML 
...
render_fig(
  ...
  fn_funnel,
  "Funnel plot of per-SNP Wald ratio estimates versus their standard errors to screen for outliers."
)

render_fig(
  "Figure S4. Leave-one-out influence plot",
  fn_loo,
  "Stability of the IVW estimate under leave-one-out suggests the inference is not driven by a single SNP."
)

render_fig(
  "Figure S5. MR-PRESSO outlier plot",
  fn_presso,
  "MR-PRESSO outlier map showing studentized residuals versus leverage (hat values) for instruments, with global and distortion test p-values and identified outliers."
)
...
S1_FN <- "Table S7 Panel summary.tsv"
S2_FN <- "Table S8 HEIDI&BF probe level hits.tsv"
...
# ---- Heatmap data: tissue × omics yields (sum passing probe records) ----
...
# Table 2-3: Tissue × omics yields (wide)
...
# Panels with ≥1 passing hit (from S1)
...
cat(sprintf(
  ...
  Under strict Bonferroni+HEIDI (panel-wise Bonferroni + p<sub>HEIDI</sub> ≥ 0.01 + n<sub>SNP,HEIDI</sub> ≥ 3),
  Ferritin retains <b>%d</b> probe–panel records (<b>%d</b> unique probes; <b>%d</b> genes),
  Delirium retains <b>%d</b> probe–panel records (<b>%d</b> unique probes; <b>%d</b> genes),
...
cat(sprintf(
  '<div class="cardx"><div class="k">Ferritin retained</div><div class="v">%d records</div><div class="k">%d probes • %d genes</div></div>',
  f_y$records, f_y$unique_probes, f_y$unique_genes
))

cat(sprintf(
  '<div class="cardx"><div class="k">Delirium retained</div><div class="v">%d records</div><div class="k">%d probes • %d genes</div></div>',
  d_y$records, d_y$unique_probes, d_y$unique_genes
))
...
cat(dl_btn("Download: Table S7 (panel summary)", paste0("data/", S1_FN)))
cat(sprintf('
<div class="figcap"><b>Table S7.</b> SMR/HEIDI panel-level summary by trait, tissue, omics layer, and QTL panel, reporting probes/genes passing Bonferroni+HEIDI and the most significant retained signal per panel.</div>'))

cat(" ")
cat(dl_btn("Download: Table S8 (BF+HEIDI probe-level hits)", paste0("data/", S2_FN)))
cat(sprintf('
<div class="figcap"><b>Table S8.</b> Probe-level SMR hits retained after panel-wise Bonferroni and HEIDI filtering, with lead SNP, effect estimates, and HEIDI diagnostics across traits and panels.</div>'))
cat('</div>')
```
...
<div class="figcap"><b>Figure S6.</b> Tissue × omics yield heatmap (sum of BF+HEIDI passing probes across panels). Heatmap of aggregated BF+HEIDI passing probe counts across panels by tissue and omics layer, shown separately for Delirium, on the left, and Ferritin, on the right.</div> 
...
<div class="figcap"><b>Figure S7.</b> Highest-yield Ferritin panels by number of BF+HEIDI passing probes. Ferritin cis-QTL panels ranked by the number of BF+HEIDI passing probes. Horizontal bars represent passing probe counts per xQTL panel.</div>
...
<div class="figcap"><b>Figure S8.</b> Top Ferritin genes based on SMR significance. Top Ferritin genes ranked by the most significant SMR association per gene (minimum p<sub>SMR</sub> across BF+HEIDI passing records).</div> 
...
<div class="figcap"><b>Figure S9.</b> Delirium retained genes (best pSMR per gene). All three signals are located at the 19q13 locus.</div>

```{r tab2_tables_main, results="asis"}

# Helper: scrollable table with caption AFTER the scroll container
render_scroll_table <- function(df, scroll_class, figcap_html) {
  cat(sprintf('<div class="%s">', scroll_class))
  cat(kbl_nice(df, caption = NULL))   # caption must NOT be inside the scroll container
  cat('</div>\n')                    # close scroll container BEFORE caption
  cat(figcap_html)
}

# Helper: non-scroll table with caption below
render_plain_table <- function(df, figcap_html) {
  cat(kbl_nice(df, caption = NULL))
  cat(figcap_html)
}

# ---------
# Table S9: Delirium retained hits (all rows)
# ---------
...
render_scroll_table(
  t1, "scroll-x",
  sprintf('\n<div class="figcap"><b>Table S9.</b> All Delirium SMR associations retained under Bonferroni+HEIDI, listing locus details, lead SNP, SMR effect, and HEIDI statistics.</div>\n')
)

# ---------
# Table S10: Strict cross-trait overlap (shared genes; APOE only here)
# ---------
...
render_scroll_table(
  t2, "scroll-x",
  sprintf('\n<div class="figcap"><b>Table S10.</b> Strict cross-trait overlap of BF+HEIDI-retained genes, showing matching retained records for the shared gene(s) across Ferritin and Delirium.</div>\n')
)

# ---------
# Table S11: Tissue × omics yields (aggregated)
# ---------
t3 <- data.table::copy(tbl_yield_matrix)

render_plain_table(
  t3,
  sprintf('\n<div class="figcap"><b>Table S11.</b> Aggregated counts of Bonferroni+HEIDI passing probes by tissue and omics layer, shown separately for Ferritin and Delirium.</div>\n')
)
...
render_scroll_table(
  tS1, "scroll-xy",
  sprintf('\n<div class="figcap"><b>Table S12.</b> Panel-level SMR/HEIDI results by trait, tissue, omics layer, and QTL panel, reporting the number of tested probes, panel-wise Bonferroni threshold, BF+HEIDI passing counts, and the top retained signal per panel.</div>\n')
)
...
render_scroll_table(
  tS2, "scroll-xy",
  sprintf('\n<div class="figcap"><b>Table S13.</b> Complete Bonferroni+HEIDI-retained SMR hit list across both traits, including panel metadata, lead SNP, SMR effect/SE, p-values, and HEIDI support.</div>\n')
)

# ---------
# Table S14 (preview) — Ferritin top BF+HEIDI passing records
# ---------
...
render_scroll_table(
  t6, "scroll-xy",
  sprintf('\n<div class="figcap"><b>Table S14 (preview).</b> Top Ferritin BF+HEIDI passing records ranked by p<sub>SMR</sub>, reporting locus details, lead SNP, SMR effect, and HEIDI statistics.</div>\n')
)
...
# Examples handled:
#   6.13×10⁻¹⁹⁸  -> 6.13e-198
#   1.28e−71      -> 1.28e-71   (unicode minus)
#   ~1.00         -> 1.00
...
# Ferritin × SLC11A2 eQTLGen row (percentages table row)
...
# Ferritin × TF Liver sQTL (abf prose)
sec_tf <- block_between(run_txt, "#### 🧬For TF Gene🧬", "(\\n### ->|\\nDelirium-pQTL|\\Z)")
...
# Delirium × APOE pQTL (primary bullets)
...
# Delirium × APOE sQTL proxies (Brain Cortex and BA9) — optional but we will display if found
sec_d_apoe_cortex <- block_between(run_txt, "Delirium × GTEx v10 sQTL \\(Brain Cortex\\) for APOE", "(\\n2\\.|\\Z)")
...
sec_d_apoe_ba9 <- block_between(run_txt, "Delirium × GTEx v10 sQTL \\(Brain BA9\\) for APOE", "(\\n3\\.|\\Z)")
...
# Delirium × CEACAM19 Cortex eQTL tables (already in runlist as pipe tables)
ce_sum   <- numify_df(table_after(run_lines, "Delirium × CEACAM19 .*— coloc summary"))
ce_top   <- numify_df(table_after(run_lines, "Top SNPs by PP\\.H4 \\(Delirium × CEACAM19, Cortex v8\\)"))
...
# ---------------------------
# 2) SuSiE results (from susie md) 
# ---------------------------

# Ferritin ↔ APOE pQTL: coloc.susie summary table
...
# Delirium ↔ APOE pQTL summary (credible sets + leads)
sec_sus_del_apoe <- block_between(sus_txt, "SuSiE–coloc\\s*run for\\s*\\*\\*Delirium \\(GWAS\\)\\*\\* vs \\*\\*APOE pQTL\\*\\*", "(\\n### 2\\.|\\n### 6\\.|\\n### 7\\.|\\Z)")
...
df_sus_del_apoe <- tibble::tibble(
  comparison = "Delirium GWAS × APOE pQTL (SuSiE)",
...
df_plot <- df_coloc_key %>%
  mutate(
    ...
    label = paste0(GWAS_trait, " × ", QTL_or_proxy),
...
# SuSiE heatmap of component pair PP.H4 (Ferritin × APOE pQTL) if table exists
p_susie_heat <- NULL
...
cat(sprintf(
  '<div class="cardx"><div class="k">Key tests summarised</div><div class="v">%d</div><div class="k">Across APOE / iron loci + proxies</div></div>',
  n_tests
))

cat(sprintf(
  '<div class="cardx"><div class="k">High PP.H4 (≥0.80)</div><div class="v">%d</div><div class="k">Evidence for a shared causal variant</div></div>',
  n_shared
))

cat(sprintf(
  '<div class="cardx"><div class="k">High PP.H3 (≥0.80)</div><div class="v">%d</div><div class="k">Evidence for distinct causal variants</div></div>',
  n_distinct
))

cat(sprintf(
  '<div class="cardx"><div class="k">APOE pQTL colocalisation</div><div class="v">%s vs %s</div><div class="k">Ferritin PP.H4 vs Delirium PP.H4</div></div>',
  ifelse(is.na(apoe_f_h4), "—", sprintf("%.3f", apoe_f_h4)),
  ifelse(is.na(apoe_d_h4), "—", sprintf("%.3f", apoe_d_h4))
))
...
print(kbl_nice(
df_show,
caption = "Table S15: Coloc.abf posterior summaries (PP.H3 vs PP.H4) across key loci and proxy panels."
))
cat(sprintf('
<div class="figcap"><b>Table S15.</b> Colocalisation posterior summaries across key loci and proxy panels, contrasting evidence for distinct signals (PP.H3) versus a shared causal variant (PP.H4).</div>'))
cat('</details>')
...
print(p_pph4)
cat('<div class="figcap"><b>Figure S10.</b> Colocalisation evidence across key tests (PP.H4). Posterior probability of a shared causal variant (PP.H4) across key GWAS–QTL/proxy comparisons. The dashed line marks the PP.H4 ≥ 0.80 threshold to denote strong shared-signal support.</div>')

cat('<hr class="sep"/>')

# ---------------------------

# Section B: Locus deep dive — APOE
...
# SuSiE table: Ferritin × APOE pQTL component pairing
...
print(kbl_nice(
sp,
caption = "Table S16: SuSiE–coloc (Ferritin GWAS × APOE pQTL): top component pairs ranked by PP.H4.abf."))}
cat(sprintf('
<div class="figcap"><b>Table S16.</b> Top SuSiE component-pair matches for Ferritin GWAS × APOE pQTL, ranked by PP.H4.abf, reporting best-aligned SNP pairs and posterior breakdown.</div>'))
...
cat('<div class="spacer"></div>')
print(p_susie_heat)
cat('<div class="figcap"><b>Figure S11.</b> Component-pair heatmap. SuSiE–coloc component-pair matrix showing PP.H4 support for shared signals between Ferritin GWAS components and APOE pQTL components. Darker cells indicate stronger colocalisation.</div>')
}

# Delirium × APOE pQTL SuSiE summary
...
print(kbl_nice(
df_del_show,
caption = "Table S17: SuSiE summary for Delirium GWAS × APOE pQTL: credible sets, lead SNPs, and the best observed component overlap (PP_shared)."
))
}

cat(sprintf('
<div class="figcap"><b>Table S17.</b> SuSiE summary for Delirium GWAS × APOE pQTL, listing credible sets, lead SNPs, and the maximum observed shared-signal probability (PP_shared).</div>'))
...
# Section C: Locus deep dive — CEACAM19 (Delirium × Cortex eQTL)
...
print(kbl_nice(
ce_sum,
caption = "Table S18: Delirium × CEACAM19 (Cortex v8) coloc.abf summary table (as recorded in runlist)."
))
}

cat(sprintf('
<div class="figcap"><b>Table S18.</b> Coloc.abf posterior probabilities (H0–H4) for Delirium GWAS × CEACAM19 eQTL in cortex (v8), as recorded in the runlist.</div>'))

if (!is.null(ce_top)) {
cat('<div class="spacer"></div>')
print(kbl_nice(
ce_top,
caption = "Table S19: Top SNPs by PP.H4 for Delirium × CEACAM19 (Cortex v8)."
))
}
cat(sprintf('
<div class="figcap"><b>Table S19.</b> SNP-level ranking by PP.H4 for the Delirium × CEACAM19 cortex colocalisation window, highlighting variants driving the shared-signal posterior.</div>'))


if (!is.null(p_ce_pph4)) {
cat('<div class="spacer"></div>')
print(p_ce_pph4)
cat('<div class="figcap"><b>Figure S12.</b> Per-SNP PP.H4 at the CEACAM19 locus. SNP-level PP.H4 contributions for Delirium × CEACAM19 cortex colocalisation, highlighting the variants that drive the shared-signal posterior in this region.</div>')
}
...
print(kbl_nice(
ce_lead,
caption = "Table S20. Lead SNP details (aligned effects) for Delirium × CEACAM19 (Cortex v8)."
))
}

cat(sprintf('
<div class="figcap"><b>Table S20.</b> Lead-variant effect alignment for Delirium GWAS and CEACAM19 cortex eQTL (betas/SE/MAF) alongside PP.H4, supporting interpretation of the colocalised signal.</div>'))
