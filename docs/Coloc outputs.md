...
#### 🧬For TOMM40 Gene🧬
I couldn't run coloc for this gene using the available files (neither in the cortex, nor in the BA9). 
...
Since there was no evidence for colocalization with the primary pQTL panel, the proxy panels were tested as well. 
...
Both traits show a real association signal in this region (H0, H1, H2 are ~0), **but** the model is  confident those signals **do not come from the same causal variant** in BA9 (PP.H3 ≈ 1, PP.H4 ~ 0). 
### -> Delirium-eQTL
#### 🧬For CEACAM19 Gene🧬
Only the primary panel used. 
